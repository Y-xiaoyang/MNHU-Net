...
def save_train_log(args, save_dir):
    ...
    with open('./MNHU-net/result/%s/train_log.txt'%save_dir ,'w') as  f:
        now = datetime.now()
        f.write("time:--")
...
def save_model(Mean_iou,best_iou, save_dir, save_prefix, train_loss, test_loss, recall, precision, epoch, net):
    if Mean_iou > best_iou:
        save_mIoU_dir = './MNHU-net/result/' + save_dir + '/' + save_prefix + '_best_IoU_IoU.log'
        save_other_metric_dir = './MNHU-net/result/' + save_dir + '/' + save_prefix + '_best_Iou_other_metric.log'
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
        best_iou = Mean_iou
        ...
        save_ckpt({
            ...
            'MIOU':Mean_iou
        }, save_path='./MNHU-net/result/' + save_dir,
            filename='mIoU' + '_' + save_prefix + '.pth.tar')
...
def save_result_for_test(dataset_dir, st_model, epochs, best_iou, recall, precision, true_positive_rate, false_positive_rate, elapsed_time):
    ...
    with open(dataset_dir + '/' +'value_result'+'/'+ st_model + '_best_other_metric.log', 'a') as f:
        ...
        for i in range(len(true_positive_rate)):
            f.write('   ')
            f.write(str(round(true_positive_rate[i], 8)))
            f.write('   ')
        f.write('\n')
        ...
        for i in range(len(false_positive_rate)):
            f.write('   ')
            f.write(str(round(false_positive_rate[i], 8)))
            f.write('   ')
        f.write('\n')
...
def make_dir(deep_supervision, dataset, model):
    ...
    if deep_supervision:
        save_dir = "%s_%s_%s_wDS" % (model,dataset,dt_string)
    else:
        save_dir = "%s_%s_%s_woDS" % (model,dataset,dt_string)
    os.makedirs('./MNHU-net/result/%s' % save_dir, exist_ok=True)            # Set save directory
    return save_dir
...
def save_Pred_GT_visulize(pred, img_demo_dir, img_demo_index, suffix, H,W):
...
def save_Pred_GT_IoU_visulize(pred, img_demo_dir, img_demo_index, suffix, H,W, blend_rate):
    ...
    plt.subplot(1, 3, 3)
    raw_3 = plt.imread(img_demo_dir + '/' + img_demo_index + suffix)
    raw = cv2.cvtColor(raw_3, cv2.COLOR_RGB2GRAY)
    predict = plt.imread(img_demo_dir + '/' + '%s_Pred' % (img_demo_index) +suffix)
    # blended_image = cv2.add(raw, predict)
    blended_image = cv2.addWeighted(raw, blend_rate, predict, (1-blend_rate), 0)
    plt.imshow(blended_image, cmap='gray')
    plt.xlabel("IoU Check", size=11)
...
def save_Pred_GT_visulize2(pred, img_demo_dir, img_demo_index, suffix, H,W):
